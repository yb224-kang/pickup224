"""
원천세 최근 3개월 데이터 수집
저장된 인증서 정보를 자동으로 읽어서 실행
딜레이: 0.1초 (tax_data_collector.py의 스마트 딜레이 사용)
"""
import sys
import json
import time
...
"""
    ...
    start_dt = f"{three_months_ago.year}{three_months_ago.month:02d}01"
    ...
    print(f"\n{'='*60}")
    print(f"[원천세 최근 3개월 데이터 수집 설정]")
    print(f"  - 인증서 수: {len(certs_list)}개")
    print(f"  - 전체 거래처 수: {len(all_clients)}개")
    print(f"  - 조회 기간: {start_dt} ~ {end_dt} (최근 3개월)")
    print(f"  - 예상 API 호출: {len(all_clients)} × 1 = {len(all_clients)}회")
    print(f"  - 딜레이: 0.1초 (tax_data_collector.py 스마트 딜레이)")
    print(f"  - 예상 시간: 약 {len(all_clients) * 0.1 / 60:.1f}분")
    print(f"{'='*60}\n")
        ...
        for idx, client in enumerate(my_clients):
            ...
            for tax_name, tax_code in TAX_MAP.items():
                ...
                if res.get("status") == "success" and res.get("count", 0) > 0:
                    success_count += 1
                    # 세목별 폴더 생성
                    tax_dir = OUTPUT_DIR / "withholding_3months"
                    ...
                    if total_errors <= 5:  # 처음 5개만 출력
                        print(f"    ⚠ {tax_name} ({biz_name}): {error_msg[:50]}", flush=True)
    
    elapsed_time = time.time() - start_time
    
    print(f"\n{'='*60}")
    print(f"[원천세 최근 3개월 데이터 수집 완료]")
    print(f"  - 총 API 호출: {total_api_calls}회")
    print(f"  - 성공한 조회: {success_count}회")
    ...
    print(f"  - 저장 위치: {OUTPUT_DIR / 'withholding_3months'}")
    print(f"{'='*60}")
